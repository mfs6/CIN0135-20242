...
void main()
{
    ...
    printf("\nl = %d\n", l); /* Dá um warning que avisa que o número l pode não caber num int que tem a marcação %d,
    aparentemente resulta em -1 sempre que não couber*/
    ...
    int i = 1024;
    printf("\nValor de i = %d\n", i);
    printf("Tamanho de i = %ld\n", sizeof(i)); // Exemplo do sizeof() que retorna o máximo de bytes que o tipo da variavel do parametro pode conter
    printf("Endereço de i = %ld\n", &i);       // Exemplo de como encontrar o endereço da memória que contem o valor da variavel (usar &)

    /*-------------------------PONTEIROS-------------------------*/
    int *p = &i; // Declaração de um ponteiro p para o int i porque usa o &
    // O ponteiro armazena o endereço da memória onde está o valor da variavel, não o próprio valor dela, porque usa o &
    ...
    int k = 1953;
    *p = k; // Pega o valor da variavel que o ponteiro aponta porque não tem o &

    printf("\nValor de k = %d\n", k);
    printf("Tamanho de k = %ld\n", sizeof(k));
    printf("Endereço de k = %ld\n", &k); // Endereço na memória de k, porque usa o &

    printf("\nValor de *p = %ld\n", *p);  // O valor de p mudou para o valor de k -> *p = k, antes ele tinha o endereço de memória de i
